...
Download **0.1.0 Beta 1 (build 19)** from [Releases](https://github.com/zamk-DAV/trackpad/releases/tag/v0.1.0-beta.1). This early beta is Developer ID-signed and notarized by Apple. Validation on other Mac models is still in progress.
...
macOS may ask whether you want to open an app downloaded from the internet on first launch.
...
When updating from a development build, the change in signing identity may require granting Input Monitoring and Accessibility permission again. Check **Input Status and Permissions** in the app. If macOS shows Flicklane as allowed but the app cannot detect permission, the permission record may still refer to its old signature. Select the existing Flicklane entry in that permission list and remove it with `−`, then use `+` to select the current `Flicklane.app` in Applications and allow it again. Quit and reopen the app. Complete any Touch ID or password request yourself.
