...
[**Flicklane 0.1.4 Beta 1 다운로드 (.zip)**](https://github.com/zamk-DAV/trackpad/releases/download/v0.1.4-beta.1/Flicklane-0.1.4-23-universal.zip) · **빌드 23 · Apple silicon + Intel**
...
- **저장된 매크로를 퀵 메뉴에 연결:** 매크로 탭의 규칙을 메뉴 한 칸에 연결하면 저장된 동작을 순서대로 실행합니다. 원본을 수정하면 메뉴에도 반영됩니다.
- **키·단축키 누르기:** 실제로 보낼 키를 직접 눌러 지정합니다. `V`, `[` 같은 단일 키부터 `⌘C`, `⌘⇧S` 같은 조합까지 설정할 수 있습니다.
- **문구 타이핑:** `Shift+A → youtube.com`처럼 실행 입력과 출력할 문구를 따로 설정합니다. 문구는 현재 입력 위치에 타이핑됩니다.
- **입력 안정성:** 문구 입력의 키 누름·뗌을 짝지었고, 실행 키의 보조 키 상태가 출력 문구에 섞이지 않도록 수정했습니다.

[0.1.4 Beta 1 전체 변경 내역과 검증 결과](docs/releases/0.1.4-beta.1.md)
...
[릴리스 파일 자동 검사](https://github.com/zamk-DAV/trackpad/actions/workflows/compatibility.yml)는 macOS 14의 Apple silicon, macOS 15·26의 Apple silicon과 Intel에서 체크섬·앱 서명·공증·리소스·실행 파일 구조를 확인합니다. **0.1.4 Beta 1의 검사 결과는 배포 후 이 페이지에 갱신합니다.** macOS 14 Intel은 이 CI에 포함되지 않습니다. 실제 제스처·권한·블루투스·잠자기 복귀를 검사하는 작업은 아닙니다.
