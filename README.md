...
**제스처, 키보드 매크로, 화면 분할, 커서 옆 퀵 메뉴를 한 앱에서.**

Flicklane(플릭레인)은 자주 쓰는 Mac 동작을 내 손에 맞게 연결하는 앱입니다. 트랙패드 동작을 녹화하거나 키 입력 순서를 정하고, 복사·붙여넣기·화면 캡처·앱 열기 같은 동작을 실행하세요.

[**Flicklane 0.1.3 Beta 1 다운로드 (.zip)**](https://github.com/zamk-DAV/trackpad/releases/download/v0.1.3-beta.1/Flicklane-0.1.3-22-universal.zip) · **빌드 22 · Apple silicon + Intel**
...
- **퀵 메뉴:** 4·5·8·9개 위치에 동작을 놓고, 키를 누른 채 커서를 움직였다가 놓으면 선택한 동작을 실행합니다. 규칙으로 열 때는 항목을 클릭합니다.
- **Fn / 🌐 실행 키:** 퀵 메뉴와 화면 분할에서 선택할 수 있습니다. 두 기능에는 서로 다른 키를 지정하세요.
- **캡처 취소:** 캡처에서 Esc를 누르면 이어지는 동작도 멈춥니다. 캡처를 취소한 뒤 이전 클립보드가 붙여넣어지던 문제를 수정했습니다.
- **텍스트와 진단:** 긴 텍스트의 이모지·확장 한자 입력을 수정하고, 진단 정보에 화면 기록 권한 상태를 추가했습니다.

[0.1.3 Beta 1 전체 변경 내역과 검증 결과](docs/releases/0.1.3-beta.1.md)

## 네 가지 기능

| 기능 | 이렇게 사용하세요 |
| --- | --- |
| [트랙패드 제스처](docs/usage.md#트랙패드-제스처) | 기본 제스처를 고르거나 접촉·탭·이동 순서를 직접 녹화합니다. 터치 사이 간격과 동작 길이의 시간 여유를 따로 조절합니다. |
| [키보드 매크로](docs/usage.md#키보드-매크로) | Q를 눌렀다 떼고 W를 누르는 순서 입력이나 Shift + 1 같은 키 조합을 연결합니다. 순서 입력의 최대 간격은 0.01~5초입니다. |
| [화면 분할](docs/usage.md#화면-분할) | 실행 키를 누른 채 포인터를 움직여 창을 절반·사분면·최대화로 배치합니다. |
| [퀵 메뉴](docs/usage.md#퀵-메뉴) | 복사, 붙여넣기, 캡처 등 자주 쓰는 동작을 커서 주변에서 선택합니다. 위치별 이름·동작을 바꾸고 실행 없이 미리 볼 수 있습니다. |
...
[한국어·영어 공유 문구](docs/share.md) · [이전 화면 자료](docs/media.md) · [전체 릴리스](https://github.com/zamk-DAV/trackpad/releases)
